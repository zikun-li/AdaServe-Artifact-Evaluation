...
#SBATCH --job-name=Proportion_llama
#SBATCH --output=llama/evaluation_out.txt
#SBATCH --error=llama/evaluation_error.err
...
export INPUT_DIR=$REPO_DIR/exps/fig10/
export OUTPUT_DIR=$REPO_DIR/results/fig10/llama/
...
if [ "$ADASERVE" == "ON" ]; then
    for PROP in $(seq $PROP_MIN $PROP_STEP $PROP_MAX); do
        ...
        export SPEC_INFER_OUTPUT_FILE=${RESULT_DIR}/prop${PROP}.txt
        export SPEC_INFER_OUTPUT_LOG=${SPEC_INFER_OUTPUT_FILE%.txt}.log
        export DATASETS_FILE=$INPUT_DIR/emission_rps${RPS}_prop${PROP}.json
        ...
        BASELINE_LATENCY_PER_TOKEN_MS=30 ENABLE_SPECSCHEDULER_SPEC_INFER=ON $TEST_SCRIPT
    done
fi
...
if [ "$VLLM" == "ON" ]; then
    for PROP in $(seq $PROP_MIN $PROP_STEP $PROP_MAX); do
        ...
        export VLLM_RESULT_FILENAME=$RESULT_DIR/prop${PROP}.txt
        export DATASETS_FILE=$INPUT_DIR/emission_rps${RPS}_prop${PROP}.json
...
if [ "$VLLM_SPEC" == "ON" ]; then
    for NST in 4 6 8; do
        for PROP in $(seq $PROP_MIN $PROP_STEP $PROP_MAX); do
            ...
            export VLLM_RESULT_FILENAME=$RESULT_DIR/prop${PROP}_nst${NST}.txt
            export DATASETS_FILE=$INPUT_DIR/emission_rps${RPS}_prop${PROP}.json
